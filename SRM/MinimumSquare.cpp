...
bool good(int len) {
	...
	REP(i, n) {
		l = x[i] - 1, r = l + len;
		st = lower_bound(x, x + n, l + 1) - x;//l+1!!!!!!!!!!!!!!!!!!!!
		en = lower_bound(x, x + n, r) - x - 1;
		REPP(j, st, en) y[j - st] = p[j].second;
		int cnt = en - st + 1;
		sort(y, y + cnt);
		REP(j, cnt) {
			d = y[j] - 1;
