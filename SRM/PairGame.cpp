...
// 	if (a == c && b == d) return 1;
// 	if (a > c || b > d) return 0;
// 	return can(a, a + b, c, d) || can(a + b, b, c, d);
// }
//Segmetation Fault
...
class PairGame {
	...
	int maxSum(int a, int b, int c, int d) {
		...
		//Segmetation Fault
		while (a && b) {
			p.push_back(1ll * a * 1234567 + b);
			if (a > b) a -= b;
			else b -= a;
		}
		while (c && d) {
			q.push_back(1ll * c * 1234567 + d);
			if (c > d) c -= d;
			else d -= c;
		}
		sort(p.begin(), p.end());
		...
		int re = -1;
		REP(i, p.size()) {
			if (std:: binary_search(q.begin(), q.end(), p[i])) re = max(1ll * re, p[i] / 1234567 + p[i] % 1234567);
		}
		p.clear(), q.clear();
		return re;
	}
